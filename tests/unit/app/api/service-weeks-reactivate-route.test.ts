...
import { reactivateServiceWeek } from "@/app/api/service-weeks/[id]/handler";
...
describe("POST /api/service-weeks/[id]/reactivate", () => {
  it("returns 401 UNAUTHENTICATED when Clerk userId is null (lookup never consulted)", async () => {
    ...
    const res = await reactivateServiceWeek(fakeReq, WEEK_ID, lookup as unknown as UserLookup);
  ...
  it("returns 401 UNAUTHENTICATED when getToken yields no JWT", async () => {
    ...
    const res = await reactivateServiceWeek(fakeReq, WEEK_ID, makeLookup("admin"));
    expect(res.status).toBe(401);
  ...
  it.each<UserRole>(["member", "set_leader", "guest"])(
    ...
    async (role) => {
      ...
      const res = await reactivateServiceWeek(fakeReq, WEEK_ID, makeLookup(role));
  ...
  it("returns 404 NOT_FOUND when the update matches no row (or belongs to another tenant)", async () => {
    ...
    const res = await reactivateServiceWeek(fakeReq, "missing-id", makeLookup("admin"));
  ...
  it("returns 200, sets is_cancelled false, and notifies each unique pending/accepted invitee", async () => {
    setUpAuth();
    ...
    const body = await res.json();
    expect(body.data.serviceWeek.isCancelled).toBe(false);
    ...
    expect(insertedPayload).toEqual([
      {
        ...
        user_id: "user-2",
        type: "service_week_reactivated",
        title: "Service week reactivated",
        body: null,
