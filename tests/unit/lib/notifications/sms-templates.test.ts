...
describe("invitationDeniedSms", () => {
  ...
  it("truncates memberName (40) and reason (60) with '...'", () => {
    ...
    expect(result).toContain(`${"m".repeat(40)}...`);
    expect(result).toContain(`Reason: ${"r".repeat(60)}...`);
  });
...
describe("practiceReminderSms", () => {
  ...
  it("omits ' — {location}' when location is null, no dangling separator", () => {
  ...
  it("truncates eventName (40) and location (40) with '...'", () => {
    ...
    expect(result).toContain(`${"e".repeat(40)}...`);
    expect(result).toContain(`${"l".repeat(40)}...`);
  });
