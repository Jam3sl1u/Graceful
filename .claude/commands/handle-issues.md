...
> **Prefer `Workflow({name: 'handle-issues'})` instead of this command for on-demand runs.** This command's Agent-tool background path was found to be unreliable in practice: it can silently no-op (narrate "kicking off work" without doing any), and a running instance is not reliably stoppable via `TaskStop`, unlike a `Workflow`-tracked run. `.claude/workflows/handle-issues.js` is a deterministic script (one issue per invocation) that doesn't have either failure mode. Keep this command around only as a foreground fallback for watching every subagent's reasoning live, e.g. while iterating on the agent prompt files themselves.
>
> **Isolated, on-demand invocation procedure (verified working):**
> 1. `EnterWorktree({name: '...'})`.
> 2. `Workflow({name: 'handle-issues'})` — no args, so it re-derives the oldest unclaimed issue itself. Runs in the background automatically.
> 3. **Do not call `ExitWorktree` and do not run any other command in the main checkout until the run's completion notification arrives.** This is critical, not optional: a `Workflow` script's cwd pinning to the worktree only holds for its *first* `agent()` call — every call after that re-resolves against whatever the session's *current* directory is at that moment. Exiting the worktree right after launch (a pattern this callout used to recommend) causes every stage past the first to silently execute against your main checkout instead, with real git operations landing in the wrong place. Verified directly via diagnostic testing: three sequential stages with real elapsed time between them all drifted to the main checkout when the worktree was exited immediately, and all three stayed correctly pinned when the session simply waited instead.
> 4. To process more than one issue in a single run without repeating this dance, loop step 2 *while still pinned* (same worktree, don't exit between issues) — up to a cap of 3, stopping early on a `blocked-on-question`, `*-failed`, or `no-op` status.
> 5. Once the run (or batch) finishes, `ExitWorktree({action: 'keep'})` if a PR needs review, or `remove` if there's nothing left to look at. Report the worktree path, branch(es), and PR URL(s) back — this also works fine unattended, since backgrounded work in this session keeps running server-side regardless of client connection state.
