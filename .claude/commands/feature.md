...
> For processing the open-issue backlog specifically (not a one-off free-text request), prefer `Workflow({name: 'handle-issues'})` — see `.claude/workflows/handle-issues.js` and the callout in `handle-issues.md`. This command remains the right tool for an ad hoc feature request that isn't tied to a GitHub issue queue.
