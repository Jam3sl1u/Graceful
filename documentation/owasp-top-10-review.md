...
| A02-4 | Info     | Resolved | Invitation `response_token` has 244 bits of entropy (two UUIDv4s, 122 bits each) in a 64-hex-char column, is unique-constrained at the DB level, and every mutating use is checked by exact equality inside a `SECURITY DEFINER` RPC before any mutation. Expiry is enforced via `invitations.response_deadline`, checked by `accept_invitation`/`deny_invitation`/`get_invitation_by_token`; the column is nullable, so an invitation created with a NULL deadline carries a bearer token that never expires. | `app/api/invitations/handler.ts` (two `crypto.randomUUID()` calls concatenated, stripped of hyphens); `20260702000003_cluster_3_scheduling_core.sql` (`response_token varchar(64) not null unique`); `20260712000002_get_invitation_by_token_rpc.sql` line 63 (`response_deadline IS NOT NULL` guard); the three RPCs listed in "Scope reviewed" | No action — a token remains resolvable via `get_invitation_by_token` (read-only) after the invitation is responded to, but `accept_invitation`/`deny_invitation` both short-circuit on `status <> 'pending'` and return the existing result without mutating again, so a leaked/reused token cannot re-trigger a state change. The NULL-deadline (never-expires) case is accepted risk, not a defect — invitations are created with a deadline by the application; no code path leaves it NULL today. |
