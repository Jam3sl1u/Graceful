...
> **Copy approved 2026-08-31** (`.pipeline/spec.md` OQ2 resolution for issue #69; this table previously had no row for that email). Fired only on a material change — an event's start time, end time, or location changing, or an attendee being assigned — never on bare create or notes-only edits.
